...
namespace JSharp
{
    public class Compiler
    {
        ...
        public static Dictionary<string, File> functionTagsEntity;
        public static Dictionary<int, List<File>> dedupFiles;
        #endregion

        private static Dictionary<string, Dictionary<ArgumentList, Function>> cachedFunctionMap;
        ...
        private static Dictionary<string, List<StructureToCMD.Block>> structureBlockCache;
        ...
        private static List<string> varGlobalWord;
        ...
        private static Regex smartClassReg = new Regex(@"^\w+\s+[\.\w]+\s*([\w\(\)\,\.\-]+\s+)*{");
        ...
        private static Regex functionTypeMaxReg = new Regex(@"^(.+)=([^>].+)");
        ...
        private static Regex entityTagsRpReg = new Regex(@"(type=#[\w\.:/]+)|(type=#\{[\w\.:/ ,]+\})");
        ...
        private static Regex unlinkedClassReg = new Regex(@"^new \w+\(.*\)");
        ...
        public static string lazyFunctionReturnFct;
        ...
        public static List<DataPackMeta.Pack.Filter.Block> FilterBlocks;
        public static string project;
                                            ...
                                            CompilerSetting setting, ProjectVersion version, string pctFolder)
        {
            ...
            FilterBlocks = new List<DataPackMeta.Pack.Filter.Block>();
            ...
            functionTagsEntity = new Dictionary<string, File>();
            ...
            varGlobalWord = new List<string>();
            objectFunc = new Dictionary<string, List<string>>();
            structureBlockCache = new Dictionary<string, List<StructureToCMD.Block>>();
            ...
            try
            {
                ...
                cachedFunctionMap = new Dictionary<string, Dictionary<ArgumentList, Function>>();
                ...
                if (Directory.Exists(path + "unzip"))
                {
                    foreach (var d in Directory.EnumerateDirectories(path + $"unzip/"))
                    {
                        GlobalDebug($"Compiling RP {d}", Color.Orange);
                        if (Directory.Exists(d + "/resourcespack"))
                        {
                            foreach (var f in Directory.EnumerateFiles(d + "/resourcespack", "*.bps", SearchOption.AllDirectories))
                            {
                                string name = f.Replace("\\", "/").Replace((d + "/resourcespack/").Replace("\\", "/"), "");
                                GlobalDebug($">> {name}", Color.Orange);
                                if (f.EndsWith(".bps"))
                                {
                ...
                FunctionCreate();
                GlobalDebug("Created Functions", Color.LimeGreen);
                ConstCreate();
                GlobalDebug("Created Constants", Color.LimeGreen);
                ScoreboardCreate();
                GlobalDebug("Created Scoreboards", Color.LimeGreen);
                StringPoolCreate();
                GlobalDebug("String Pool Create", Color.LimeGreen);
                MuxClose();
                ...
                GlobalDebug("Closing File", Color.LimeGreen);
        ...
        public static void updateFormater()
        {
            try
            {
                ...
                Formatter.varGlobalWord = varGlobalWord;
                Formatter.loadDict();
            }
            catch { }
        }
                ...
                if (line != "")
                {
                    ...
                    if (isInLazyCompile == 0)
                    {
                        ...
                        if (smartContains(line, ';'))
                        {
                            ...
                            {
                                if ((smartExtract(subline) == "}" || autoIndented == 1) && !inGenericStruct && isInLazyCompile == 0 && limit != context.currentFile())
                                {
                                    context.currentFile().Close();
                                }
                                else
        ...
        public static string parseLine(string text, bool ignoreUnparsed = false)
        {
            ...
            try
            {
                ...
                if (jsonIndent == 0)
                {
                    ...
                    {
                        autoIndented = 0;
                        return parseLine(text.Substring(1));
                    }
                }
                ...
                if ((text.StartsWith("return") && text.Contains("(") && text.Contains(")")) || text.StartsWith("return "))
                {
                    ...
                    {
                        args = smartSplit(text.Substring(7), ',');
                    }
                ...
                {
                    //class
                    if (classReg.Match(text).Success)
                    {
                    ...
                    //switch
                    if (switchReg.Match(text).Success)
                    {
                    ...
                    //smart case
                    if (text.Contains("->") && switches.Count > 0)
                    {
                        return InstSmartCase(text);
                    }
                    //with
                    if (withReg.Match(text).Success)
                    {
                    ...
                    //positioned
                    if (positonedReg.Match(text).Success)
                    {
                    ...
                    //aligned
                    if (alignReg.Match(text).Success)
                    {
                        string args = getArg(text);
                    ...
                    //while
                    if (whileReg.Match(text).Success)
                    {
                        string arg = getArg(text);
                ...
                if (jsonIndent == 0)
                {
                    ...
                    //enum set
                    if (enumReg.Match(text).Success)
                    {
                        return InstEnum(text);
                    }
                    //enum file
                    if (enumFileReg.Match(text).Success)
                    {
                        return InstEnumFile(text);
                    }
                    //unlinked class
                    if (unlinkedClassReg.Match(text).Success)
                    {
                        return InstUnlinkedClass(text);
                    }
                    //function def
                    Match funMatch = funcReg.Match(text);
                    if (funMatch.Success && illagal_op.All(x => x != funMatch.Groups["function_name"].Value))
                    {
                        return InstFunc(text);
                    }
                    //blocktag set
                    if (blocktagReg.Match(text).Success)
                    {
                        return InstBlockTag(text);
                    }
                    //entitytag set
                    if (entitytagReg.Match(text).Success)
                    {
                        return InstEntityTag(text);
                    }
                    //itemTag set
                    if (itemtagReg.Match(text).Success)
                    {
                        return InstItemTag(text);
                    }
                    //smart class
                    if (smartClassReg.Match(text).Success)
                    {
                        return InstSmartClass(text);
                    }
                }
                //comp int set
                ...
                {
                    //int set
                    if (varInstReg.Match(text).Success || functionTypeReg.Match(text).Success)
                    {
                        return InstVar(text);
                    }
                    //int add
                ...
                if (text != "" && !text.StartsWith("}") && !ignoreUnparsed)
                {
                    GlobalDebug("Unparsed line:'" + text + "' in "+ context.GetVar(), Color.Yellow);
                }
                return "";
            }
            catch (Exception e)
        ...
        public static string compVarReplace(string line)
        {
            if (!requireReg.Match(line).Success && !indexedReg.Match(line).Success && line.Contains("$"))
            {
                ...
                keys.Add("$stackTrace");
                dic.Add("$stackTrace", context.GetVar());
                ...
                foreach (string key in keys)
                {
                    ...
                    {
                        reg = new Regex(@"\\b" + key.Replace(".", "\\.") + "\\b");
                        if (key.Contains("$"))
                            reg = new Regex("\\" + key.Replace(".", "\\."));
        ...
        {
            var dic = context.compVal[context.compVal.Count - 1];
            if (a.type == Type.ENTITY)
            {
                ...
                dic[a.name]= context.GetEntitySelector(smartExtract(args[i]));
            }
            else if (a.type == Type.INT || a.type == Type.FUNCTION || a.type == Type.FLOAT)
            {
                ...
                if (valVar != null)
                {
                    dic[a.name + ".enums"]= valVar.enums;
                    dic[a.name + ".type"]= valVar.GetTypeString();
                    dic[a.name + ".name"]= valVar.gameName;
                    dic[a.name + ".scoreboard"]= valVar.scoreboard();
                    dic[a.name + ".scoreboardname"]= valVar.scoreboard().Split(' ')[1];
                }
                if (a.type == Type.FUNCTION)
                {
                    try
                    {
                        dic[a.name + ".name"]= functions[context.GetFunctionName(smartExtract(args[i]))][0].gameName;
                    }
                    catch { }
                }
                dic[a.name]= smartExtract(args[i]);
            }
            ...
            else if (a.type == Type.PARAMS)
            {
                ...
                dic[a.name]= param + ")";
                dic[a.name + ".raw"] = paramsRaw;
                dic[a.name + ".count"]= c.ToString();
            }
        ...
        public static string functionDesugar(string text)
        {
            ...
            Match m2 = functionTypeMaxReg.Match(text);
            text = m2.Success ? m2.Groups[1].Value : text;
            ...
            return text+ (m2.Success?"="+m2.Groups[2].Value:"");
        }
        ...
        public static string ifelseDetect(string text)
        {
            ...
            Stack<int> ifIndentPIndex = new Stack<int>();
            ...
            foreach (char c in text)
            {
                ...
                else if (c == '}' && !isInString)
                {
                    ...
                    wrd = lastWord.ToString();
                    lastWord.Clear();
                    if (ifIndentIndex.Count > 0 && ifIndentIndex.Peek() == indent && parindent == ifIndentPIndex.Peek())
                    {
                ...
                if (wrd == "if")
                {
                    ...
                    ifIndex.Push(i);
                    ifIndentPIndex.Push(parindent);
                    wrd = "";
                }
                else if (wrd == "else" && startIndent == indent)
                {
                    ifsIndex.Add(start);
                    wrd = "";
                }
        ...
        public static string ifelseDetectOld(string text)
        {
            StringBuilder lastWord = new StringBuilder();
        ...
        public static string import(string text)
        {
            ...
            {
                GlobalDebug($"Already Importated: {text}", Color.DarkGreen);
                return "";
            }
            string output = "";
        ...
        public static bool tryImport(string folder, string lib, bool fu, out string msg)
        {
            ...
            foreach (string text in paths)
            {
                if (System.IO.File.Exists(folder + text + ".tbms"))
                {
                    ...
                    GlobalDebug(msg, Color.DarkGreen);
                ...
                {
                    importRes(folder + text + ".bpz", fu, text);
                ...
                {
                    if (Directory.Exists(path+ $"unzip/{text}")) Directory.Delete(path + $"unzip/{text}", true);
                    ZipFile.ExtractToDirectory(folder + text + ".zip", path + $"unzip/{text}");
                    List<string> functions = Directory.EnumerateFiles(path + $"unzip/{text}", "*.mcfunction", SearchOption.AllDirectories)
                             .Select(x => x.Replace("\\","/").Replace(path.Replace("\\", "/") + $"unzip/{text}/data", "").Replace("functions/", "").Replace("/",".").Replace(".mcfunction",""))
                    ...
                    Directory.Delete(path + $"unzip/{text}", true);
        ...
        public static void importRes(string file, bool fu, string import)
        {
            string path = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) + "/";
            import = import.Replace("/", ".");
            try
            {
                if (Directory.Exists(path + $"unzip/{import}")) Directory.Delete(path + $"unzip/{import}", true);
                Directory.CreateDirectory(path + $"unzip/{import}");
                ZipFile.ExtractToDirectory(file, path + $"unzip/{import}");

                if (Directory.Exists(path + $"unzip/{import}/scripts"))
                {
                    compileFiles(Directory.EnumerateFiles(path + $"unzip/{import}/scripts", "*.bps", SearchOption.AllDirectories)
                             .Select(x => new File(x.Replace(path + $"unzip/{import}/scripts","imported").Replace("\\", "//"), System.IO.File.ReadAllText(x))).ToList(), false, false);
                }
                if (Directory.Exists(path + $"unzip/{import}/resources"))
                {
                    var res = Directory.EnumerateFiles(path + $"unzip/{import}/resources", "*.*", SearchOption.AllDirectories)
                ...
                if (Directory.Exists(path + $"unzip/{import}/structures"))
                {
                    foreach (var f in Directory.EnumerateFiles(path + $"unzip/{import}/structures", "*.nbt", SearchOption.AllDirectories))
                    {
                        string name = PathConverter.Local(f, path + $"unzip/{import}/structures/");
                        Form1.SafeCopy(f, path + "imported_dp/structure/" + name);
                    }
                }
                if (Directory.Exists(path + $"unzip/{import}/resourcespack"))
                {
                    foreach (var f in Directory.EnumerateFiles(path + $"unzip/{import}/resourcespack", "*.*", SearchOption.AllDirectories))
                    {
                        string name = PathConverter.Local(f, path + $"unzip/{import}/resourcespack/");
                        if (!f.EndsWith(".bps"))
                        {
                            Form1.SafeCopy(f, path + "imported_rp/" + name);
                        }
                        else if (!System.IO.File.Exists(path + $"imported_rp/{name}"))
                        {
                            Form1.SafeCopy(f, path + $"imported_rp/{name}");
                        }
                        else
                        {
                            int i = 0;
                            name = name.Replace(".bps", "");
                            while (System.IO.File.Exists(path + $"imported_rp/{name}_{i}.bps"))
                            {
                                i++;
                            }
                            Form1.SafeCopy(f, path + $"imported_rp/{name}_{i}.bps");
                        }
                    }
                }
            }
        ...
        public static string modVar(string text)
        {
            ...
            if (left.Length == 2 && right.Length == 2 && left[0] == right[1] && left[1] == right[0])
                return swap(left);
            ...
            else
            {
                ...
                if (left.Length == 1 && right.Length > 1)
                {
                    ...
                    if (splitted.Length == 2 && context.isEntity(left[0]))
                    {
                    ...
                    else if (left[0].StartsWith("@"))
                    {
                        for (int i = 0; i < right.Length; i++)
                        {
                            ...
                            AddToFunctionTag(fun, left[0].Substring(1));
                        }
                        return "";
                    }
                    else
        ...
        public static string swap(string[] vars)
        {
            var v1 = GetVariableByName(vars[0]);
            var v2 = GetVariableByName(vars[1]);
            if (v1.type != v2.type) throw new Exception("Invalid Swap Type");
            if (v1.type == Type.ARRAY)
            {
                if (v1.arraySize != v2.arraySize) throw new Exception("Invalid Swap Array Size");
                StringBuilder output = new StringBuilder();
                for (int i = 0; i < v1.arraySize; i++)
                {
                    output.Append(Core.VariableOperation(GetVariableByName($"{vars[0]}.{i}"), GetVariableByName($"{vars[1]}.{i}"), "><"));
                }
                return output.ToString();
            }
            else if (v1.type == Type.STRUCT)
            {
                var str = structs[v1.enums];
                if (str.isClass)
                {
                    return Core.VariableOperation(v1, v2, "><");
                }
                else
                {
                    StringBuilder output = new StringBuilder();
                    ...
                    {
                        output.Append(Core.VariableOperation(GetVariableByName($"{vars[0]}.{field.name}"), GetVariableByName($"{vars[1]}.{field.name}"), "><"));
                    }
                    return output.ToString();
                }
            }
        ...
        private static string eval(string val, Variable variable, Type ca, string op = "=", int recCall = 0) 
        {
            ...
            if (valVar == null && ca != Type.FUNCTION
                 && !(ca == Type.ENUM && enums[variable.enums].Contains(val.ToLower()))
                 && !val.Contains("(") && context.IsFunction(val.Replace(" ", "")))
            {
            ...
            bool containsSB = smartContains(val, '[');
            bool arrayVarRegRes = arrayVarReg.Match(val).Success;
            bool isEntity = context.isEntity(val);
            bool arrayFuncRegRes = arrayFuncReg.Match(val).Success;
            ...
            else if (spaceSplitted.Length >= 3 && operators_comp.All(x => spaceSplitted[1] != x) &&
                !(spaceSplitted[1] == "=>" && ca == Type.FUNCTION))
            {
            ...
            else if (!isEntity && containsSB && arrayVarRegRes && arrayFunc3Reg.Match(val).Success)
            {
                return functionEval(val, new string[] { variable.gameName }, op);
            }
            else if (!isEntity && containsSB && arrayVarRegRes && !arrayFuncRegRes)
            {
            ...
            else if (ca == Type.STRUCT)
            {
                ...
                if (getStruct(val.Replace("(", " ")) != null)
                {
                    ...
                    if (structs[stru] == stru1)
                    {
                        ...
                        output += parseLine(variable.gameName + ".__init__" + val.Substring(val.IndexOf('(')));
                    }
                    else
                    {
                        if (structs[stru].isStatic)
                ...
                {
                    void withOperatorFunction(string name)
                    {
                        if (val == "{")
                        {
                            output += parseLine(variable.gameName + $".{name}" + val);
                        }
                        else
                        {
                            output += parseLine(variable.gameName + $".{name}(" + val + ")");
                    ...
                    {
                        withOperatorFunction("__mult__");
                    }
                    else if (op == "+=")
                    {
                        withOperatorFunction("__add__");
                    }
                    else if (op == "-=")
                    {
                        withOperatorFunction("__sub__");
                    }
                    else if (op == "/=")
                    {
                        withOperatorFunction("__div__");
                    }
                    else if (op == "&=")
                    {
                        withOperatorFunction("__and__");
                    }
                    else if (op == "|=")
                    {
                        withOperatorFunction("__or__");
                    }
                    else if (op == "^=")
                    {
                        withOperatorFunction("__xor__");
                    }
                    else if (op == "%=")
                    {
                        withOperatorFunction("__mod__");
                    }
                }
            }
            ...
            else if (ca == Type.BOOL)
            {
                ...
                else if (isEntity && smartContains(val, '.'))
                {
                    if (op == "=")
                        return "execute store result score " + variable.scoreboard() + " run " + NBT_Data.parseGet(context.ConvertEntity(val), 1) + '\n';
                    else
        ...
        {
            Condition v = getConditionSplit(text, rec);
            return Core.Condition(v);
        }
        private static Condition getConditionSplit(string text, int rec = 0)
        {
            ...
            for (int i = 0; i < arg.Length; i++)
            {
                ...
                {
                    conditions.Add(getCondOr(arg[i], ++rec));
                }
                else
                {
                    ...
                    if (arg[i].StartsWith("!"))
                    {
                        Condition in1 = getCond(arg[i].Substring(1, arg[i].Length - 1), rec);

                        if (in1.GetCondition() == ConditionAlwayTrue)
                        {
                            conditions.Add(new JavaConstantCondition(false));
                        }
                        else if (in1.GetCondition() == ConditionAlwayFalse)
                        {
                            
                        }
                        else
                        {
                            conditions.Add(Core.ConditionInverse(in1));
                        }
                    }
                    else
                    {
                        Condition in1 = getCond(arg[i], rec);
                        if (in1.GetCondition() == ConditionAlwayTrue)
                        {
            ...
            return new JavaConditionList(conditions);
        }
        private static Condition getCondOr(string text, int rec = 0)
        {
            List<Condition> conditions = new List<Condition>();

            int idVal = If.GetEval(context.GetFun());

            string prep = parseLine("bool __eval" + idVal.ToString() + "__ = " + text.Replace("&&", "&").Replace("||", "|"));

            conditions.Add(new SingleJavaCondition("", prep, "", true));
            conditions.Add(getCond("__eval" + idVal.ToString() + "__", rec));

            return new JavaConditionList(conditions);
        }
        private static Condition getCond(string text, int rec = 0)
        {
            int tmpI;
            double tmpF;
            int idVal = If.GetEval(context.GetFun());
            ...
            if (context.isEntity(text) && !smartContains(text, '.') && context.IsValudEntitySelector(text))
            {
                return Core.ConditionEntity(context.GetEntitySelector(text));
            }
            else if (text.StartsWith("tag("))
            {
                string arg = getArgs(text).Select(x => $"tag={x}").Aggregate((x, y) => $"{x},{y}");
                return Core.ConditionEntity("@s[" + arg + "]");
            }
            else if (text.StartsWith("entity.tag("))
            {
                string arg = getArgs(text).Select(x => $"tag={x}").Aggregate((x, y) => $"{x},{y}");
                return Core.ConditionEntity("@s[" + arg + "]");
            }
            else if (text.StartsWith("block("))
            {
                ...
                if (args[0].Contains("#") && !args[0].Contains(":"))
                {
                    ...
                    {
                        string[] part = smartSplitJson(args[0], ' ');
                        if (part.Length == 4)
                        {
                            string tagName = part[3].Replace("#", "");
                            if (tagName.StartsWith("{"))
                            {
                                InstBlockTag($"blocktags __generated_{idVal}__ = {tagName.Replace("{", "").Replace("}", "")}");
                                tagName = $"__generated_{idVal}__";
                            }
                            string tag = Core.FormatTagsPath(context.GetBlockTags(tagName));
                            output = part[0] + " " + part[1] + " " + part[2] + " #" + tag;
                        }
                        else
                        {
                            string tagName = part[0].Replace("#", "");
                            if (tagName.StartsWith("{"))
                            {
                                InstBlockTag($"blocktags __generated_{idVal}__ = {tagName.Replace("{", "").Replace("}", "")}");
                                tagName = $"__generated_{idVal}__";
            ...
            else if (lower.StartsWith("__isvar("))
            {
                string[] arg = getArgs(text);
                if (arg.All(x => GetVariableByName(x, true) != null))
                {
                    return new JavaConstantCondition(true);
                }
                else
                {
                    return new JavaConstantCondition(false);
                }
            }
            else if (lower.StartsWith("__equals("))
            {
                string[] arg = getArgs(text);
                if (arg[0] == arg[1])
                {
            ...
            else if (lower.StartsWith("__isnb("))
            {
                string[] arg = getArgs(text);
            ...
            else if (lower.StartsWith("__isint("))
            {
                string[] arg = getArgs(text);
            ...
            else if (lower.StartsWith("__isstring("))
            {
                string[] arg = getArgs(text);
            ...
            else if (lower.StartsWith("__isentity("))
            {
                string[] arg = getArgs(text);
            ...
            else if (lower.StartsWith("__hasspace("))
            {
                string[] arg = getArgs(text);
            ...
            else if (lower.StartsWith("__debug"))
            {
                return compilerSetting.opti_ShowDebug?new JavaConstantCondition(true): new JavaConstantCondition(false);
            }
            else if (lower.StartsWith("__exception"))
            {
                return compilerSetting.opti_ShowDebug ? new JavaConstantCondition(true) : new JavaConstantCondition(false);
            }
            else if (lower.StartsWith("__isblocktags("))
            {
                string[] arg = getArgs(text);
                if (arg.All(x => context.GetBlockTags(x, true) != null))
                {
                    return new JavaConstantCondition(true);
                }
                else
            ...
            else if (lower.StartsWith("__isitemtags("))
            {
                string[] arg = getArgs(text);
                if (arg.All(x => context.GetItemTags(x, true) != null))
                {
                    return new JavaConstantCondition(true);
                }
                else
            ...
            else if (lower.StartsWith("__isentitytags("))
            {
                string[] arg = getArgs(text);
                if (arg.All(x => context.GetEntityTags(x, true) != null))
                {
                    return new JavaConstantCondition(true);
                }
                else
            ...
            {
                if (functionTags.ContainsKey(lower.Substring(1)) && functionTags[lower.Substring(1)].Count > 0)
                {
                    return new JavaConstantCondition(true);
                }
                else
            ...
            else if (context.GetPredicate(text, true) != null && text.Contains("(") && text.Contains(")"))
            {
                ...
                return new SingleJavaCondition("predicate " + pred.get(getArg(text)));
            }
            else if (text.Contains("=="))
            {
                ...
                if (arg[1] == "null")
                {
                    return appendPreCond(Core.ConditionInverse(Core.CompareVariable(GetVariableByName(arg[0]), GetVariableByName(arg[0]), "=", selector1, selector1)), pre);
                }

                if (t == Type.STRUCT)
                {
                    if (structs[var.enums].HasMethod("__equal__"))
                    {
                        return getCond(var.gameName + ".__equal__(" + var.gameName + ")");
                    }
                    else
                    {
                        Variable var2 = null;
                        if (context.GetVariable(arg[1], true) != null)
                        {
                            var2 = GetVariableByName(arg[1]);
                            if (var2.enums == var.enums)
                            {
                                var struc = structs[var.enums];
                                var conds = struc.fields.Select(x => getCond($"{var.gameName}.{x.name}=={var2.gameName}.{x.name}", rec + 1)).ToList();

                                return new JavaConditionList(conds);
                            }
                            else
                            {
                                throw new Exception($"{var.gameName} doesn't have an __equal__ method!");
                            }
                        }
                        throw new Exception($"{var.gameName} doesn't have an __equal__ method!");
                    }
            ...
            else
            {
                ...
                if (op == "")
                {
                    if (smartEmpty(text) == "true") { return new JavaConstantCondition(true); }
                    if (smartEmpty(text) == "false") { return new JavaConstantCondition(false); }
                    double a = 0;
                    if (double.TryParse(smartEmpty(text), out a) && a > 0) { return new JavaConstantCondition(true); }
                    if (double.TryParse(smartEmpty(text), out a) && a <= 0) { return new JavaConstantCondition(false); }
                ...
                if (op == "<=")
                {
                    if (td) { return new JavaConstantCondition(true); }
                    if (tc && ta <= tb) { return new JavaConstantCondition(true); } else if (tc) { return new JavaConstantCondition(false); }
                }
                if (op == "<")
                {
                    if (td) { return new JavaConstantCondition(false); }
                    if (tc && ta <= tb) { return new JavaConstantCondition(true); } else if (tc) { return new JavaConstantCondition(false); }
                }
                if (op == ">=")
                {
                ...
                else if ((t == Type.ENUM) && enums[var.enums].valuesName.Contains(smartEmpty(arg[1]).ToLower()))
                {
                ...
                else if(t == Type.STRUCT)
                {
                    ...
                    pre += parseLine("bool cond_" + idVal3.ToString() + " = " + (selector1 != "" ? selector1 + "." : "") + arg[0]+"."
                        +FunctionOpName(op)+"("+arg[1]+")");
                    entity1 = selector1 != "";
                    selector1 = "";
                    var = GetVariableByName("cond_" + idVal3);

                    return appendPreCond(Core.CompareVariable(var, 1, ">=", selector1), pre);
                }
                else if (t == Type.FLOAT && double.TryParse(arg[1], out tmpF))
                {
        ...
        private static Condition appendPreCond(Condition text, string val)
        {
            return new JavaConditionList(new List<Condition>() { new SingleJavaCondition("", val, ""), text });
        }
        #endregion
        ...
        {
            if (cachedFunctionMap.ContainsKey(name) && !func.isAbstract)
            {
                cachedFunctionMap.Remove(name);
            }
            else if (cachedFunctionMap.ContainsKey(name) && func.isAbstract)
            {
                return;
            }
        ...
        public static Function GetFunction(string funcName, string[] input_args, bool lambda = false)
        {
            ...
            Type[] argType = new Type[input_args.Length];
            bool skip = false;
            for (int i = 0; i < input_args.Length; i++)
            {
                try
                {
                    argType[i] = getExprType(input_args[i]);
                }
                catch
                {
            ...
            if (!skip)
            {
                ArgumentList arg = new ArgumentList(funcName, argType, lambda);
                if (cachedFunctionMap.ContainsKey(funcName) && cachedFunctionMap[funcName].ContainsKey(arg))
                {
                    return cachedFunctionMap[funcName][arg];
                }
                if (!cachedFunctionMap.ContainsKey(funcName))
                {
                    cachedFunctionMap.Add(funcName, new Dictionary<ArgumentList, Function>());
                }
                var f = GetFunctionUncached(funcName, input_args, lambda);
                
                cachedFunctionMap[funcName][arg] = f;

                return f;
            }
            else
            {
                return GetFunctionUncached(funcName, input_args, lambda);
            }
        }
        public static Function GetFunctionUncached(string funcName, string[] input_args, bool lambda = false)
        {
            if (functions[funcName].Count == 1)
            {
                return functions[funcName][0];
        ...
        public static void AddFunctionToGRPFile(Variable variable, Function func, string grp, File fFile)
        {
            ...
            foreach (Variable outputVar in variable.outputs)
            {
                if (func.file.UnparsedFunctionFile || func.isAbstract)
                {
                    func.file.addParsedLine("__mux__." + grp + ".ret_" + i.ToString() + "=" + func.outputs[i].gameName);
                }
                else
                {
                    func.file.AddLine(parseLine("__mux__." + grp + ".ret_" + i.ToString() + "=" + func.outputs[i].gameName));
                }
                i++;
            }
            functDelegatedSwitch[grp].Add(id.ToString(), casecontent.ToArray());
        }
        ...
        public static void AddToFunctionTag(Function func, string tag, string className = null)
        {
            ...
            {
                if (!functionTagsEntity.ContainsKey(tag))
                {
                    File entityTag = new File("__tags__/" + tag.Replace(".", "/").ToLower()+"__at_entity__");
                    functionTagsEntity[tag] = entityTag;
                    entityTag.use();
                    files.Add(entityTag);
                    f.AddLine(Core.As(context.GetEntitySelector("@e[type=#__class_entity__,tag=__class__]"))+Core.CallFunction(entityTag));
                }
        ...
        public static string InstCompilerVar(string text)
        {
            ...
            else if (value.StartsWith("hex"))
            {
                if (text.ToLower().StartsWith("define"))
                    value = compVarReplace(value);
                string[] argget = getArgs(value);
                string newvalue = int.Parse(compVarReplace(argget[0])).ToString("X3");
                context.compVal[context.compVal.Count - 1][name] = newvalue;
                return "";
            }
            else if (value.StartsWith("hash"))
            {
                if (text.ToLower().StartsWith("define"))
                    value = compVarReplace(value);
                string[] argget = getArgs(value);
                string newvalue = compVarReplace(argget[0]).GetHashCode().ToString();
                context.compVal[context.compVal.Count - 1][name] = newvalue;
                return "";
            }
            ...
            else if (value.StartsWith("indexfromenum"))
            {
                ...
                context.compVal[context.compVal.Count - 1][name] = enu.IndexOf(smartExtract(argget[1].ToLower())).ToString();
            ...
            else if (value.StartsWith("fromenum"))
            {
                ...
                context.compVal[context.compVal.Count - 1][name] = enu.GetValueOf(smartExtract(argget[1].ToLower()));
                foreach (string f in sortedField)
                {
                    context.compVal[context.compVal.Count - 1][name + "." + f] = enu.GetFieldOf(smartExtract(argget[1].ToLower()), f);
                }
                return "";
            }
            else if (value.StartsWith("fromconst"))
            {
                ...
                context.compVal[context.compVal.Count - 1][name] = var.constValue;
                return "";
            }
            ...
            else if (value.StartsWith("$") && ((structStack.Count > 0 && !isInStructMethod) || structCompVarPointer != null) && op == "=")
            {
                ...
                {
                    value = evalDesugar(compVarReplace(value));

                    structCompVarPointer[name] = evalDesugar(compVarReplace(value));
                }
                else if (structStack.Count > 0 && !isInStructMethod)
                {
                    structStack.Peek().compField[name] = value;
                }
                return "";
            }
        ...
        public static string InstVar(string text)
        {
            ...
            {
                isStatic |= structStack.Peek().isStatic;
            }
            ...
            foreach (string v in smartSplit(vari, ','))
            {
                ...
                if (ca == Type.ARRAY)
                {
                    ...
                    if (structStack.Count > 0 && !isInStructMethod)
                    {
                        ...
                        if (structStack.Peek().isClass && false)
                        {
        ...
        public static string InstFunc(string text)
        {
            ...
            if (func.StartsWith("@"))
            {
                tags.Add(func.Replace("@", ""));
                func = "lambda_" + Lambda.GetID(context.GetFun());
                isLambda = true;
            }
            ...
            if (isOverride)
            {
                ...
                else if (functions[funcID][0].isAbstract && functions[funcID][0].isStatic)
                {
            ...
            fFile.UnparsedFunctionFile = (!lazy && structStack.Count == 0 && !isAbstract) || (isStatic && !isAbstract && !lazy);
            fFile.UnparsedFunctionFileContext = isStatic? context.GetVar().Replace("__struct__", ""):context.GetVar();

            if (structStack.Count > 0 && !isStatic && !isAbstract)
            {
            ...
            foreach (string z in args)
            {
                ...
                if (c.Contains(' '))
                {
                    ...
                    //GlobalDebug(context.GetInput() + "---" + context.GetVar() + "----" + a+" "+c, Color.Yellow);
            ...
            if (!(structStack.Count > 0 && !isStatic) && !(structStack.Count > 0 && structStack.Peek().isAbstract))
            {
                function.AddToTags();
            }
        ...
        public static string InstWith(string[] text, string fText, bool adjPackageClear = false)
        {
            if (context.isEntity(text[0]))
            {
                ...
                if (text.Length > 2)
                {
                    Condition v = getConditionSplit(text[2]);
                    if (smartEmpty(text[1]) == "true")
                        pre = v.GetSetup().LinesToText("\n", true) + Core.AsAt(context.GetEntitySelector(text[0]), v.GetCondition());
                    else
                        pre = v.GetSetup().LinesToText("\n", true) + Core.As(context.GetEntitySelector(text[0]), v.GetCondition());
                }
                else
                {
        ...
        public static string InstAt(string[] text, string fText)
        {
            if (context.isEntity(text[0]))
            {
                ...
                if (text.Length > 1)
                {
                    Condition v = getConditionSplit(text[1]);
                    pre = v.GetSetup().LinesToText("\n",true) + Core.At(context.GetEntitySelector(text[0]), v.GetCondition());
                }
                else
                {
        ...
        public static string InstBlockTag(string text)
        {
            ...
            string name = (compilerSetting.tagsFolder ? context.GetVar() : "") +
                            subfields.Last();
            if (subfields.Last().StartsWith("."))
                name = subfields.Last().Substring(1);
        ...
        public static string InstStruct(string text)
        {
            ...
            string[] constructArg = null;

            if (name.Contains("(")){
                constructArg = getArgs(name);
            ...
            if (!isStatic)
                ...
                context.Sub(name, new File(name, "", "struct"));

            Structure stru = new Structure(name, parent, interfaces, isPrivate, isClass, isStatic);

            if (classEntity != null) stru.classEntity = classEntity;
            ...
            if (constructArg != null && !isStatic)
            {
                constructArg.ToList().ForEach(x => parseLine(x, false));
                var param = constructArg.Where(x => x.Split(' ').Length > 1).Select(x => {
                    var s = x.Split(' ');
                    return "int $" + s.Skip(1).ToArray().LinesToText(" ");
                }).ToArray();
                var body = constructArg.Where(x => x.Split(' ').Length > 1).Select(x => {
                    var s = x.Split(' ');
                    return "this."+s[1] + " = $" + s[1];
                });

                preparseLine($"lazy __init__({param.LinesToText(", ")})"+"{");
                body.ToList().ForEach(x => preparseLine(x));
                preparseLine("}");
            }
            else if (constructArg != null && isStatic)
            {
                string args = constructArg.ToList().LinesToText(", ", false);
                preparseLine($"{name}.__init__({args})");
            }
            if (!og_text.Contains("{"))
            {
                preparseLine("}");
        ...
        {
            string pck = smartEmpty(text.Substring(8));
            currentPackage = pck;
            if (currentPackage != ".")
            {
        ...
        public static string InstJsonFile(string text)
        {
            ...
            if (currentParsedFile != null && currentParsedFile.resourcespack)
            {
                ...
                if (text.Contains("{"))
                {
                    resourcespackFiles[resourcespackFiles.Count - 1].AddLine(text.Substring(text.IndexOf("{")));
                }
                jsonIndent = text.Count(x => x == '{') - text.Count(x => x == '}');
                return "";
            }
            else
            {
                ...
                if (text.Contains("{"))
                {
                    jsonFiles[jsonFiles.Count - 1].AddLine(text.Substring(text.IndexOf("{")));
                }
                jsonIndent = text.Count(x => x == '{') - text.Count(x => x == '}');
                return "";
            }
        ...
        public static string InstPredicateFile(string text)
        {
            ...
            string[] args = getArgs(text);
            string func = context.GetFun().ToLower();
            File f = new File("predicates/" + func + name, "", "json");
            Predicate p = new Predicate(func + name, args, f);
            string key = context.GetVar().ToLower() + name;
            ...
            string subName = key.Substring(key.IndexOf(".") + 1);
            funcDef.Add(subName);
        ...
        public static string InstUnlinkedClass(string text)
        {
            Regex reg = new Regex(@"^new\s+");
            text = reg.Replace(text, "");
            string cls = getStruct(text.Replace("(", " "));
            string arg = getArg(text);
            string part2 = text.Substring(getOpenCharIndex(text, '('));
            if (cls == null)
                throw new Exception("NULL");
            if (!structs.ContainsKey(cls))
                throw new Exception($"No {cls} class");
            if (!structs[cls].isClass)
                throw new Exception("Not A Class!");
            structs[cls].generate("@fake_var%", false, new Variable("@fake_var%", "@fake_var%", Type.VOID), part2 + (part2.EndsWith("{")?"}":""));
            ...
            string func = structs[cls].UnlinkedClassInit.gameName.Replace(":", ".").Replace("/", ".");
            func = func.Substring(0, func.LastIndexOf(".")) + ".__init__";
            
            string value = parseLine($"{func}{part2}");
            adjPackage.Pop();
            return value;
        }
        public static string InstSmartClass(string text)
        {
            text = smartExtract(text);
            if (text[text.Length - 1] == '{')
                text = text.Substring(0, text.Length - 1);
            ...
            return InstStruct("class "+ split.Skip(1).Aggregate((x,y)=>x+" "+y)+$" extends {split[0]}"+"{");
        }
        #endregion
        ...
        public static string functionEval(string text, string[] outVar = null, string op = "=", int rec = 0)
        {
            ...
            {
                string arg = getArg(text);
                string[] args = smartSplitJson(arg, ',', -1, true);
                ...
                if (var != null && var.type == Type.STRUCT && structs[var.enums].methodsName.Contains("__call__") && !muxAdding)
                {
                    func += ".__call__";
                }
                string funcName = context.GetFunctionName(func);
                ...
                if (funObj.lazy)
                {
                    ...
                    if (args.Length > funObj.args.Count && !funObj.args.Any(x => x.type == Type.PARAMS || x.type == Type.JSON))
                        throw new Exception($"To much Argument for {funObj.gameName}: recieve {args.Length} ({args.Aggregate((x,y)=>$"{x},{y}")}) expected: {funObj.args.Count}");
                    
                    bool inplace = funObj.attributes != null && funObj.attributes.Contains("inplace");

                    string stackID = funObj.GetID(context.GetVar()).ToString();
                    string lazyconte =
                        funObj.variableStruct != null ?
                        context.GetVar() + (inplace ? "" : "c_" + funObj.name+"_"+ Math.Abs(funObj.variableStruct.GetHashCode()).ToString() + "_"+ stackID + "."):
                        context.GetVar() + (inplace?"":"c_" + funObj.name + stackID + ".");

                    if (outVar != null)
                    {
                        for (int k = 0; k < funObj.outputs.Count; k++)
                        {
                            if (outVar.Length > k)
                                addLazyVal($"{lazyconte}ret_{k}", context.GetVariable(outVar[k]));
                        }
                    }
                    foreach (Argument a in funObj.args)
                    {
                        if (args.Length <= i)
                        {
                            ...
                            else if (endWithAccollade && a.type == Type.FUNCTION)
                            {
                                ...
                                var dic = context.compVal[context.compVal.Count - 1];
                                dic[a.name + ".name"]= functions[context.GetFunctionName(anonymusFuncName)][0].gameName;
                                if (a.name.StartsWith("$"))
                                    dic[a.name]= context.GetFunctionName(anonymusFuncName);
                                else
                                    addLazyVal(lazyconte+a.name, context.GetFunctionName(anonymusFuncName));
                        ...
                        else
                        {
                            ...
                            else if (a.name.StartsWith("$"))
                            {
                                if (a.type == Type.FUNCTION)
                                {
                                    string fctarg = args[i];
                                    bool isFunc = fctarg.Contains("(") && IsFunction(fctarg);
                                    Function funcArgObj = isFunc ? functions[context.GetFunctionName(getFunctionName(fctarg))][0] : null;
                                    if (isFunc && funcArgObj.lazy && funcArgObj.outputs.Count == 1 && funcArgObj.outputs[0].type == Type.FUNCTION)
                                    {
                                        functionEval(fctarg);
                                        string[] o = addCompVar(a, new string[] { lazyFunctionReturnFct }, 0);
                                        if (o != null)
                                        {
                                            init += o[0];
                                            clear += o[1];
                    ...
                    if (!inplace)
                    {
                        string nSubFileName = funObj.variableStruct != null ?
                            "c_" + funObj.name + "_" + Math.Abs(funObj.variableStruct.GetHashCode()).ToString() + "_" + stackID :
                            "c_" + funObj.name + stackID;
                        context.Sub(nSubFileName, new File(nSubFileName, "", "lazyfunctioncall"));
                    }
        ...
        {
            string ouput = "";
            if (arg.Length == 1 && lazyCall.Count > 0 && lazyCall.Peek().outputs[0].type == Type.FUNCTION)
                lazyFunctionReturnFct = context.GetFunctionName(arg[0]);
            if (lazyEvalVar.Count > 0 && lazyOutput.Count > 0)
            {
                ...
                if (lazyOutput.Peek().Count == 0 && false)
                {
        ...
        {
            StringBuilder output = new StringBuilder("#=================================================#\n");
            output.AppendLine(functionDesc.Replace("\"\"\"", ""));
            output.AppendLine("#" + func.gameName);
            functionDesc = "";
            output.AppendLine("#Argument:");

            foreach (Argument arg in func.args)
            {
                output.AppendLine($"# - {arg.gameName} ({arg.uuid}): {arg.GetFancyTypeString()}");
            }
        ...
        public static string[] smartSplitJson(string text, char c, int max = -1, bool ignoreGreater = false)
        {
            List<string> output = new List<string>();
            int ind = 0;
            StringBuilder stringBuilder = new StringBuilder(text.Length);
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                else if (text[i] == '<' && c == ',' && !inString && !ignoreGreater)
                {
                    ind += 1;
                    stringBuilder.Append(text[i]);
                }
        ...
        {
            string[] args = smartSplitJson(getArg(text), ',', - 1, true);
            for (int i = 0; i < args.Length; i++)
            {
        ...
        public static string FunctionOpName(string name)
        {
            if (name == "<=") return "__smaller_or_equals__";
            if (name == "<") return "__smaller__";
            if (name == ">=") return "__bigger_or_equals__";
            if (name == ">") return "__bigger__";
        ...
        public static string jsonAppend(string src, string text)
        {
            ...
            if (text.StartsWith("{"))
            {
                if (text == "{}")
                    return src;
                var elem = JsonMerger.GetObject(src);
                elem.Merge(JsonMerger.GetObject(text), false);
            ...
            while (changed)
            {
                ...
                while (fi < files.Count)
                {
                    if ((files[fi].UnparsedFunctionFile && !files[fi].notUsed))
                    {
                        ...
                        if ((DateTime.Now - fstartTime).TotalMilliseconds > 100)
        ...
        public static void FunctionOptimisation()
        {
            ...
            {
                var lineCount = f.content.Count(x=>x=='\n');
                if (lineCount == 1 && f.content[0] != '#' && f.name != "load" && f.name != "main")
                {
        ...
        public class Function : CompilerObject
        {
            ...
            public Dictionary<string, int> SwitchNumber = new Dictionary<string, int>();
            ...
            public bool isClassRepMethod = false;
            ...
            public Function CopyTo(string gameName, string name, File f)
            {
                ...
                newfunc.isTicking = isTicking;
                newfunc.tags = tags;
                newfunc.isLoading = isLoading;
                newfunc.isPrefix = isPrefix;
                newfunc.isInfix = isInfix;
                ...
                if (isStatic)
                {
                    string path = gameName.Contains(":") ? gameName.Substring(gameName.IndexOf(":") + 1) : gameName;

                    newfunc.file = new File(path, f.content, f.type);
                    files.Add(newfunc.file);
                    newfunc.file.UnparsedFunctionFile = true;
                    newfunc.file.UnparsedFunctionFileContext = gameName.Replace("/", ".").Replace(":", ".")+".";
                    newfunc.file.parsed = f.parsed;
                    newfunc.file.parsed_end = f.parsed_end;
                    newfunc.file.function = newfunc;

                    AddFunction(gameName.Replace("/", ".").Replace(":", "."), newfunc);
            ...
            public void AddToTags()
            {
                if (isLoading)
                {
                    AddToFunctionTag(this, "loading");
                    file.use();
                    if (callStackDisplay)
                        callTrace += "\"load\"->\"" + gameName + "\"\n";
                }

                if (isTicking)
                {
                    AddToFunctionTag(this, "ticking");
                    file.use();
                    if (callStackDisplay)
                        callTrace += "\"main\"->\"" + gameName + "\"\n";
                }
                ...
                if (isHelper)
                {
                    file.use();
                    if (callStackDisplay)
                        callTrace += "\"helper\"->\"" + gameName + "\"\n";
                }


                foreach (string tag in tags)
                {
                    AddToFunctionTag(this, tag);
                    if (callStackDisplay)
                        callTrace += "\"@" + tag.ToLower() + "\"->\"" + gameName + "\"\n";
                }
            }
        }
        public class Structure : CompilerObject
        {
            ...
            public Dictionary<string, Function> methodsMaps = new Dictionary<string, Function>();
            ...
            public string classEntity;
            public Function UnlinkedClassInit;
            public string UnlinkedClassContext;

            public Structure(string name, Structure parent, List<Structure> interfaces, bool isPrivate, bool isClass, bool isStatic)
            {
                this.name = name;
                this.parent = parent;
                ...
                if (parent != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            string varName = context.toInternal(context.GetVar() + v.name);
                            if (v.isStatic) varName = varName.Replace("__struct__" + name, name);
                            Variable variable = new Variable(v.name, varName, v.type, v.entity, v.def);
                            ...
                            {
                                variable.SetEnum(v.enums);
                                structs[v.enums].generate(v.name, v.entity, variable);
                            }
                    ...
                    foreach (Function f in parent.methods)
                    {
                        if (isStatic)
                        {
                            Function newFun = f.CopyTo(context.GetFun().Replace("/" + name, "") + name + "/" + f.name, f.name, f.file);
                            methods.Add(newFun);
                        }
                        else
                        {
                            Function newFun = f.CopyTo(context.GetFun().Replace("/__struct__" + name, "") + name + "/" + f.name, f.name, f.file);
                            methods.Add(newFun);
                        }
                    }
            ...
            public void addMethod(Function element)
            {
                ...
                if (!element.isVirtual || element.isStatic)
                {
                    ...
                    methodsMaps[element.name] = element;
                }
                else
                {
            ...
            public void generateFunction(Function fun, Variable varOwner, string v, bool parentClass)
            {
                ...
                function.isClassMethod = isClass;
                function.isClassRepMethod = parentClass;
                ...
                if (fun.isLoading)
                {
                    if (isClass && parentClass)
                    {
                        ...
                        AddToFunctionTag(function, "loading", map);
                    }
                    else if (!isClass)
                    {
                ...
                if (fun.isTicking)
                {
                    ...
                    {
                        string map = classOffuscationMapAdd(name);
                        AddToFunctionTag(function, "ticking", map);
                    }
                    else if (!isClass)
                    {
                ...
                #endregion
                context.Sub(fun.name + dif, fFile);
                ...
                fFile.notUsed = (!fun.isLoading && !fun.isHelper && !fun.isTicking && fun.tags.Count == 0) || (isClass && !parentClass);
                ...
                foreach (string line in fun.file.parsed)
                {
                    ...
                    else
                    {
                        if (context.compVal.Count > 0 && !(dualCompVar.Match(line).Success && structInstCompVar))
                        {
                            fFile.parsed.Add(compVarReplace(thisReg.Replace(line, thisDef)));
                        }
                        else
                        {
                            fFile.parsed.Add(thisReg.Replace(line, thisDef));
                        }
                    }
                }
                if (isClass  && (!parentClass || fun.name == "__init__") && !fun.isStatic)
                {
                    fFile.parsed.Add("}");
                }
            }
            ...
            {
                if (isClass && classEntity != null)
                {
                    parseLine($"entitytags .__class_entity__ = {classEntity}");
                }
                ...
                if (isClass && !parentClass)
                {
                    if (representative == null)
                    {
                        ...
                        var rpName = $"__class__";

                        string fName = context.GetVar() + rpName;
                        representative = new Variable(rpName, fName, Type.STRUCT, false, "__class_id__");
                        ...
                        generate(rpName, false, representative, instArg, true);
                ...
                if (!isClass || parentClass)
                {
                    foreach (Variable strVar in fields)
                    {
                        if (!strVar.isStatic)
                        {
                            try
                            {
                                if (isClass && !isStatic && false)
                                {
                                    strVar.ToEntity();
                                    strVar.LinkCopyTo(context.GetVar() + strVar.name);
                                    strVar.parent = varOwner;
                                    strVar.inClass = isClass;
                                }
                                else
                                {
                ...
                var dic = context.compVal[context.compVal.Count - 1];
                dic["$this"] = varOwner.uuid;
                dic["$this.lower"] = varOwner.uuid.ToLower();
                dic["$this.upper"] = varOwner.uuid.ToUpper();
                dic["$this.enums"] = varOwner.enums;
                dic["$this.type"] = varOwner.GetTypeString();
                dic["$this.name"] = varOwner.gameName;
                dic["$this.scoreboard"] = varOwner.scoreboard();
                dic["$this.scoreboardname"] = varOwner.scoreboard().Split(' ')[1];
                ...
                if (instArg != null)
                {
                    ...
                    foreach (Function fun in methods)
                    {
                        ...
                        {
                            generateFunction(fun, varOwner, v, parentClass);
                        }
                    }
                    foreach (Function fun in methods)
                    {
                ...
                foreach (Function fun in methods)
                {
                    if (((fun.name != "__base_init__" && fun.name != "__init__") || instArg == null) && !fun.isStatic)
                    {
                        generateFunction(fun, varOwner, v, parentClass);
                    }
                }
                ...
                typeMaps.Pop();
                structGenerating = false;
                if (HasMethod("__call__") && !parentClass)
                {
                    var function = methodsMaps["__call__"];
                    var name = varOwner.uuid.Substring(varOwner.uuid.IndexOf(".") + 1, varOwner.uuid.Length - varOwner.uuid.IndexOf(".") - 1);
                    if (function.args.Count == 1 && function.args[0].type == Type.FUNCTION)
                        funcDefF.Add(name);
                    else if (function.args.Count > 1 && function.args[function.args.Count - 1].type == Type.FUNCTION)
                        funcDefM.Add(name);
            ...
            public static void DerefObject(Variable variable)
            {
                ...
                preparseLine("with(@e[tag=__class__],false,__CLASS__==__class_pointer__ && __CLASS__ != null){");
                preparseLine("__ref--");
                ...
                {
                    foreach (Variable v in structs[variable.enums].fields)
                    {
                        if (v.type == Type.STRUCT && structs[v.enums].isClass)
                        {
                            preparseLine($"{variable.gameName}.{v.name} = null");
                        }
                    }
                    preparseLine("/kill @s");
                }
        ...
        public class Variable : CompilerObject
        {
            ...
            public Variable(string name, string gameName, Type type, bool entity = false, string def = "dummy")
            {
                ...
                if (attributes != null && attributes.Any(x => x.StartsWith("offuscationforced")))
                {
                    string v = attributes.First(x => x.StartsWith("offuscationforced"));
                    compilerSetting.forcedOffuscation[gameName] = v.Split('=')[1];
                }
            ...
            public void LinkCopyTo(string newName)
            {
                ...
                if (this.type == Type.ARRAY)
                {
                    ...
                    {
                        functions[f.gameName].ForEach(x => x.LinkCopyTo(newName));
                    }
                ...
                if (this.type == Type.STRUCT)
                {
                    ...
                    foreach (Function method in structs[this.enums].methods)
                    {
                        functions[gameName + "." + method.name].ForEach(x => x.LinkCopyTo(newName));
                    }
                }
        ...
        public class Enum
        {
            ...
            public class EnumField
            {
                ...
                public EnumField(Enum parent, string type, string name, string defaultVal)
                {
                    ...
                    if (type != "json")
                    {
                        ...
                        functionGet.file.UnparsedFunctionFile = true;
                    }
                }
                ...
                public void AddValue(int index, EnumValue value)
                {
                    if (type != "json")
                    {
                        ...
                        {
                            functionGet.file.addParsedLine($"if ({multiplexer.gameName}=={index}){"{"}{output.gameName}={value.fields[name]}{"}"}");
                        }
                        else if (defaultVal != "")
                        {
                            functionGet.file.addParsedLine($"if ({multiplexer.gameName}=={index}){"{"}{output.gameName}={defaultVal}{"}"}");
                        }
                        else
            ...
            public void Add(string value)
            {
                ...
                foreach (string field in fields)
                {
                    ...
                    if (smartContains(field, '=') && subField.Length > 1)
                    {
        ...
        public class Switch : Component
        {
            ...
            public void CreateVariable(string text)
            {
                ...
                copyFrom = int.TryParse(text, out int _)?null:GetVariableByName(smartExtract(text), true);
                ...
                if (casesUnit.Count > subTreeSize)
                {
                    ...
                    for (int i = 0; i < Math.Ceiling((casesUnit.Count * 1.0) / subTreeSize); i++)
                    {
                        ...
                        if (variable.type == Type.FLOAT)
                            ...
                            (casesUnit[iMin].value/1000d).ToString() + ".." +
                            (casesUnit[iMax].value/1000d).ToString())
                            + cmd + "\n";
                        else
                            text += getCondition(variable.gameName + "==" +
            ...
            public Case Add(string cond, string cmd)
            {
                ...
                if (variable.type == Type.ENUM)
                {
                    ...
                    {
                        casesUnit.Add(new SingleCase(enums[variable.enums].valuesName.IndexOf(smartEmpty(cond).ToLower()), cmd));
                        return casesUnit[casesUnit.Count - 1];
                    }
                    else
                    {
                        casesRange.Add(new SingleCase(cond, cmd));
                ...
                else if (variable.type == Type.INT)
                {
                    ...
                    {
                        casesUnit.Add(new SingleCase(int.Parse(cond), cmd));
                        return casesUnit[casesUnit.Count - 1];
                    }
                    else
                    {
                ...
                else if (variable.type == Type.FLOAT)
                {
                    ...
                    {
                        casesUnit.Add(new SingleCase((int)(double.Parse(cond) * compilerSetting.FloatPrecision), cmd));
                        return casesUnit[casesUnit.Count - 1];
                    }
                    else
                    {
            ...
            public Case Add(string cond, string[] cmd)
            {
                ...
                if (variable.type == Type.ENUM)
                {
                    if (variable.enums != null && enums[variable.enums].valuesName.Contains(smartEmpty(cond).ToLower()))
                    {
                        casesUnit.Add(new MultipleCase(enums[variable.enums].valuesName.IndexOf(smartEmpty(cond).ToLower()), cmd));
                        return casesUnit[casesUnit.Count - 1];
                    }
                    else
                    {
                        casesRange.Add(new MultipleCase(cond, cmd));
                        return casesRange[casesRange.Count - 1];
                    }
                }
                else if (variable.type == Type.INT)
                {
                    if (int.TryParse(cond, out int _))
                    {
                        casesUnit.Add(new MultipleCase(int.Parse(cond), cmd));
                        return casesUnit[casesUnit.Count - 1];
                    }
                    else
                    {
                ...
                else if (variable.type == Type.FLOAT)
                {
                    if (double.TryParse(cond, out double _))
                    {
                        casesUnit.Add(new MultipleCase((int)(double.Parse(cond) * compilerSetting.FloatPrecision), cmd));
                        return casesUnit[casesUnit.Count - 1];
                    }
                    else
                    {
            ...
            public abstract class Case
            {
                ...
                public abstract string GetText(Variable v, bool ranged);
            }
            public class SingleCase: Case
            {
                public string cmd;

                public SingleCase(int value, string cmd)
                {
                    this.value = value;
                    this.cmd = cmd;
                }

                public SingleCase(string valueStr, string cmd)
                {
                    this.valueStr = valueStr;
                    this.cmd = cmd;
                }

                public override string GetText(Variable v, bool ranged)
                {
                    if (ranged)
                    {
                        return getCondition(v.gameName + "==" + valueStr) + cmd + "\n";
                    }
                    else
                    {
                        if (v.type == Type.FLOAT)
                            return getCondition(v.gameName + "==" + ((value * 1d) / compilerSetting.FloatPrecision).ToString()) + cmd + "\n";
                        else
                            return getCondition(v.gameName + "==" + value.ToString()) + cmd + "\n";
                    }
                }
            }
            public class MultipleCase : Case
            {
                public string[] cmd;

                public MultipleCase(int value, string[] cmd)
                {
                    this.value = value;
                    this.cmd = cmd;
                }

                public MultipleCase(string valueStr, string[] cmd)
                {
                    this.valueStr = valueStr;
                    this.cmd = cmd;
                }
                ...
                public override string GetText(Variable v, bool ranged)
                {
                    if (ranged)
                    {
                        return cmd.Select(x => getCondition(v.gameName + "==" + valueStr) + x).LinesToText("\n");
                    }
                    else
                    {
                        if (v.type == Type.FLOAT)
                        {
                            string d = ((value * 1d) / compilerSetting.FloatPrecision).ToString();
                            return getCondition(v.gameName + "==" + d) + cmd + "\n";
                        }
                        else
                        {
                            return cmd.Select(x => getCondition(v.gameName + "==" + value.ToString()) + x).LinesToText("\n");
                        }
                    }
                }
            }
        ...
        public class ArgumentList
        {
            ...
            public ArgumentList(string name, Type[] args, bool lambda)
            {
                this.func = name;
                this.args = args;
                this.lambda = lambda;
            ...
            {
                return args.Select(x => x.GetHashCode()).Aggregate(func.GetHashCode(), (x, y) => x + y)+ lambda.GetHashCode();
            }
            public override bool Equals(object obj)
            {
                ...
                {
                    ArgumentList p = (ArgumentList)obj;
                    if (lambda != p.lambda)
                    {
                        return false;
                    }
                    if (p.args.Length != args.Length || func != p.func)
                    {
                        return false;
                    }
                    for (int i = 0; i < args.Length; i++)
        ...
        public class CompilerSetting
        {
            ...
            public string structuresSource = "";
            public Dictionary<string, DateTime> structuresSources = new Dictionary<string, DateTime>();
            ...
            public bool opti_FileCleanUp = true;
        ...
        public class File
        {
            ...
            public File AddLine(string cont)
            {
                ...
                {
                    if (cont.EndsWith("\n"))
                        contentbuilder.Append(cont);
                    else
                        contentbuilder.AppendLine(cont);

                    if (cont.Contains('\n'))
                        lineCount += cont.Count(x => x =='\n');
                    else
                        lineCount++;
                }
                return this;
            ...
            public void generate(string value, object data = null, string file = null)
            {
                isInLazyCompile = 0;
                ...
                foreach (string l in parsed)
                {
                    ...
                    if (data != null && data is Enum)
                    {
                        Enum enums = (Enum)data;
                        List<string> sortedField = new List<string>();
                    ...
                    else if (data != null && data is StructureToCMD.Block)
                    {
                        StructureToCMD.Block block = (StructureToCMD.Block)data;
                        line = line.Replace(var + ".x", $"{block.x}");
                        line = line.Replace(var + ".y", $"{block.y}");
                        line = line.Replace(var + ".z", $"{block.z}");

                        line = line.Replace(var + ".id", $"{block.block_id}");
                        line = line.Replace(var + ".properties", $"{block.block_data_dict}");
                        line = line.Replace(var + ".states", $"{block.block_data_mc}");
                    }
                    else if (value.StartsWith("("))
                    {
                        string[] argget = getArgs(value);
            ...
            public void forgenerate()
            {
                ...
                else if (enumGen != null && enumGen.StartsWith("substring("))
                {
                    string[] args = getArgs(enumGen);
                    string filter = "*.*";
                    if (args.Length > 1) { filter = "*." + extractString(args[1]); }
                ...
                else if (enumGen != null && enumGen[0] == '@')
                {
                    var tag = enumGen.Replace("@", "");
                    CreateFunctionTag(tag);
                    genAmount = functionTags[tag].Count;
                    foreach (string value in functionTags[tag])
                    {
                        generate(value);
                    }
                }
                else if (enumGen != null && enumGen.StartsWith("structure("))
                {
                    string structs = extractString(getArg(enumGen));
                    if (!structureBlockCache.ContainsKey(structs))
                    {
                        string ProjectPath = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) + "/";

                        if (System.IO.File.Exists(projectFolder + "/structures/" + structs + ".nbt"))
                        {
                            structureBlockCache.Add(structs, StructureToCMD.Convert(projectFolder + "/structures/" + structs + ".nbt"));
                        }
                        else if (Directory.Exists(ProjectPath + "imported_dp/structure/") &&
                            System.IO.File.Exists(ProjectPath + "imported_dp/structure/" + structs + ".nbt"))
                        {
                            structureBlockCache.Add(structs, StructureToCMD.Convert(ProjectPath + "imported_dp/structure/" + structs + ".nbt"));
                        }
                        else
                        {
                            throw new Exception($"Structure {structs} doesn't exists.");
                        }
                    }

                    genAmount = structureBlockCache[structs].Count;
                    foreach (var value in structureBlockCache[structs])
                    {
                        generate("" + value, value);
                    }
                }
                else if (enumGen != null && enumGen.StartsWith("blocktags("))
                {
                    var tag = blockTags[context.GetBlockTags(getArg(enumGen))];
                    genAmount = tag.values.Count;
                    foreach (string value in tag.values)
                    {
                        generate(value, null);
                    }
                }
                else if (enumGen != null && enumGen.StartsWith("entitytags("))
                {
                    var tag = blockTags[context.GetEntityTags(getArg(enumGen))];
                    genAmount = tag.values.Count;
                    foreach (string value in tag.values)
                    {
                        generate(value, null);
                ...
                else if (enumGen != null && enumGen.StartsWith("itemtags("))
                {
                    var tag = blockTags[context.GetItemTags(getArg(enumGen))];
                    genAmount = tag.values.Count;
                    foreach (string value in tag.values)
                    {
                        generate(value, null);
                ...
                else if (enumGen != null && enumGen.StartsWith("\""))
                {
                    genAmount = enumGen.Length;
                    foreach (char value in getString(enumGen))
                    {
                        generate("" + value, null);
                    }
                }
                else if (enumGen != null && context.GetBlockTags(enumGen, true) != null)
                {
                    var tag = blockTags[context.GetBlockTags(enumGen)];
                    genAmount = tag.values.Count;
                    foreach (string value in tag.values)
                    {
                        generate(value, null);
                ...
                else if (enumGen != null && context.GetEntityTags(enumGen, true) != null)
                {
                    var tag = entityTags[context.GetEntityTags(enumGen)];
                    genAmount = tag.values.Count;
                    foreach (string value in tag.values)
                    {
                        generate(value, null);
                ...
                else if (enumGen != null && context.GetItemTags(enumGen, true) != null)
                {
                    var tag = itemTags[context.GetItemTags(enumGen)];
                    genAmount = tag.values.Count;
                    foreach (string value in tag.values)
                    {
                        generate(value, null);
                ...
                else if (enumGen != null && int.TryParse(enumGen, out int _))
                {
                    genAmount = int.Parse(enumGen);
            ...
            {
                contentbuilder.Replace("run  execute", "run execute");
                content = scoreboardDef.ToString() + start + content + ending;
                ending = "";
                start = "";

                if (compilerSetting.opti_FileCleanUp)
                {
                ...
                if (type == "struct")
                {
                    ...
                    if (!structStack.Peek().methodsName.Contains("__init__"))
                    {
                        preparseLine("def __init__(){");
                        preparseLine("}");
                    }
                ...
                bool with_class = CompiledFunction != null && CompiledFunction.isClassMethod && !CompiledFunction.isClassRepMethod && name == CompiledFunction.file.name+ "/w_0";
                if ((type == "if" || (type == "with" && !cantMergeWith && !with_class) || type == "at") && lineCount == 1 && !content.StartsWith("#"))
                {
                ...
                {
                    ((Switch.SingleCase)switchcase).cmd = content;
                    valid = false;
                }
                if (type == "switch")
                {
                ...
                if ((type == "if" || type == "case" || type == "with"/* || (function != null && compilerSetting.opti_LambdaCleanUp)*/) && compilerSetting.opti_FileCleanUp)
                {
                    bool found = false;
                    if (dedupFiles.ContainsKey(content.GetHashCode()))
                    {
                        ...
                        foreach (File file in lst)
                        {
                            if (file.content == content && file.valid && !file.notUsed)
                            {
                                ...
                                {
                                    var sc = ((Switch.SingleCase)switchcase);
                                    sc.cmd = sc.cmd.Replace(Core.CallFunction(this), Core.CallFunction(file));
                                ...
                                if (function != null && valid && !notUsed && !file.isLazy && !isLazy)
                                {
                                    files.Where(x => !x.notUsed && x.valid).ToList().ForEach(x => x.content = x.content
                                            .Replace(Core.CallFunction(this), Core.CallFunction(file)));
                                    if (functDelegatedFile.ContainsKey(this.function.gameName))
                                    {
                                        functDelegatedFile[this.function.gameName] = file;
                                    }
                                    found = true;
                                    valid = false;
                                    file.multiUsed = true;
            ...
            public void Compile()
            {
                ...
                if (compilerSetting.opti_ExportComment)
                {
                    context.currentFile().AddStartLine(GenerateInfo(function));
                }
                try
                {
                ...
                for (int i = 0; i < parsed_end.Count; i++)
                {
                    string line = parsed_end[i];
                    currentLine = i+parsed.Count;
                    try
                    {
                        preparseLine(line);
                    }
        ...
        public class Context
        {
            ...
            public string ReplaceEntityTags(string value)
            {
                ...
                {
                    string tag = smartSplitJson(m.Value, '#')[1];

                    if (tag.StartsWith("{"))
                    {
                        int idVal = If.GetEval(context.GetFun());
                        InstEntityTag($"entitytag __generated_{idVal}__ = {tag.Replace("{", "").Replace("}", "")}");
                        tag = $"__generated_{idVal}__";
                    }
                    tag = GetEntityTags(tag);

                    value = regReplace(value, m, "type=#" + Core.FormatTagsPath(tag.Contains(".")?tag:(Project + "." +tag)));
                }
                return value;
            }
            ...
            public bool IsValudEntitySelector(string value, bool single = true)
            {
                value = ReplaceEntityTags(value);
                if (value.Contains("@"))
                {
            ...
            {
                var match = entityReg.Match(smartEmpty(value));
                if (match.Success)
                {
                    return true;
                }
